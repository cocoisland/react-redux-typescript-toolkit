...
const App = function() {
  ...
  const handleCreateNewTodo = (e: FormEvent<HTMLFormElement>): void => {
    e.preventDefault();
    if (!newTodoInput.length) return; // nothing to do if Todo list is zero

    dispatch(createTodoActionCreator({desc: newTodoInput}) );
    setNewTodoInput(''); // after dispatching newTodo input, reset Todo list to empty
  ...
  const handleToggle = (): void => {
    ...
    dispatch(toggleTodoActionCreator({
      id:selectedTodoId, isComplete:!selectedTodo.isComplete
    }));
  };
